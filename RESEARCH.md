...
| 39 | **Repeat mode searches counts descending and reports the binding requirement.** `solve_repeat` tries the requested count first and walks down to the largest achievable. When it falls short it removes requirements one at a time and re-solves to find which one blocks squad M+1, and says "the size of the available pool, not any single requirement" when no removal helps, rather than naming an innocent rule. |
